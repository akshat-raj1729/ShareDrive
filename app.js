...
// database connection and server creation
async function main(){
...
app.post('/upload',requireAuth , checkUser, upload.single('file'), async (req, res) => {
  try {
    ...
    console.error('Error uploading file akshat', error);
    res.status(500).send('Error uploading file akshat');
  }
});
...
app.get("/shared",requireAuth,checkUser, async (req,res) =>{
  ...
  try {
    ...
    // .populate() is useful for displaying additional details about the users who shared the files.
...
app.get("/permanentDelete", checkUser, async (req,res) => {
  try {
    ...
    const filePath = path.join(__dirname, file.filePath);
    fs.unlinkSync(filePath);// for local storage
